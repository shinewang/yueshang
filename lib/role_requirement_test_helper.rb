...
module RoleRequirementTestHelper
  ...
  # Makes sure a account can access the given action
  #
  # Example:
  #
  #   assert_account_can_access(:quentin, "index")
  # 
  def assert_account_can_access(account, actions, params = {})
    assert_account_access_check(true, account, actions, params)
  end
  
  # Makes sure a account cant access the given action
  #
  # Example:
  #
  #   assert_account_cant_access(:quentin, "destroy", :listing_id => 1)
  # 
  def assert_account_cant_access(account, actions, params = {})
    assert_account_access_check(false, account, actions, params)
  end
  
  # Check a list of accounts against a set of actions with parameters.
  # 
  # Parameters:
  #   accounts_access_list - a hash where the key is the label for a fixture, and the value is a boolean.
  ...
  #     [:show, :edit], 
  #     {:listing_id => 1})
  def assert_accounts_access(accounts_access_list, actions, params = {})
    accounts_access_list.each_pair {|account, access| 
      assert_account_access_check(access, account, actions, params)
    }
  end
  
  alias :assert_account_cannot_access :assert_account_cant_access

private
  def assert_account_access_check(should_access, account, actions, params = {})
    ...
    (Array===actions ? actions : [actions]).each { |action|
      ...
      login_as account
      if should_access
        assert request_passes_role_security_system?(action, params), "request to #{@controller.class}##{action} with account #{account} and params #{params.inspect} should have passed "
      else
        assert ! request_passes_role_security_system?(action, params), "request to #{@controller.class}##{action} with account #{account} and params #{params.inspect} should have been denied"
      end
    }
  end
