...
module RoleRequirementSystem
  ...
  module RoleSecurityClassMethods
    ...
    #    require_role "admin", :only => :update, :unless => "current_account.authorized_for_listing?(params[:id]) "
    #
    # Valid options
    #
    ...
    # This is the core of RoleRequirement.  Here is where it discerns if a account can access a controller or not./
    def account_authorized_for?(account, params = {}, binding = self.binding)
      ...
      self.role_requirements.each{| role_requirement|
        ...
        passed = false
        roles.each { |role|
          passed = true if account.has_role?(role)
        } unless (account==:false || account==false)
  ...
  module RoleSecurityInstanceMethods
    ...
    def access_denied
      if logged_in?
        ...
        respond_to do |format|
					format.html do
            #store_location
						#flash[:notice] = "You don't have permission to complete this action."
	        ...
	        format.js do
	          ...
	          render :text => "你没有执行该操作的权限。", :status => '401 Unauthorized'
					end
				end
      else
        super
    ...
    def check_roles       
      return access_denied unless self.class.account_authorized_for?(current_account, params, binding)
      
      true
    end
    
  protected
    # receives a :controller, :action, and :params.  Finds the given controller and runs account_authorized_for? on it.
    ...
    def url_options_authenticate?(params = {})
      ...
      klass.account_authorized_for?(current_account, params, binding)
    end
  end
end