class UserFailure < ActiveRecord::Base
	...
	def self.failure_check(remote_ip)
		find(:first, :conditions =>
			['remote_ip = ? and count >= ? and updated_at >= ?', remote_ip, 4, 1.hour.ago])
	end

  def self.record_failure(remote_ip, http_user_agent, failure_type, username = nil)
    logger.warn("登录失败:#{username}@#{Time.now.to_s} #{remote_ip}")
  ...
  def record
    "#{id}: #{updated_at}来自#{remote_ip}的用户#{count}次尝试使用#{username}登录"
  end
end